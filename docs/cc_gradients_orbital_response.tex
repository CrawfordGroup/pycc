...
Hessian $\mathbf{G}$.  Its right-hand side needs the source response $\partial
X_{ai}/\partial y = \partial I'_{ia}/\partial y - \partial I'_{ai}/\partial y$,
hence the perturbed generalized Fock $\partial I'/\partial y$, and the response
$\partial\mathbf{G}/\partial y$ of the orbital Hessian.  The perturbed
...
The following ingredients are common to both perturbed-orbital choices:
\begin{enumerate}
\item The {\em undifferentiated} relaxed densities $\tilde D_{pq}$,
$\Gamma_{pqrs}$, and $I_{pq}$, assembled exactly as in the first-derivative
summary (\S\ref{sec:comp-summary-1}); they carry the first perturbation $x$
implicitly.
...
$\partial\Gamma_{pqrs}/\partial y$, $\partial I_{pq}/\partial y$, assembled as in
\S\ref{sec:2nd-density-response} from the once-perturbed wave-function parameters
and the perturbed Z-vector (the $(2n+1)$ rule; no second-order wave-function
response is needed).
\end{enumerate}
