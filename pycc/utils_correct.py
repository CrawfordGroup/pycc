...
class helper_diis(object):
    ...
    def add_error_vector(self, t1, t2):
        if isinstance(t1, torch.Tensor):
            ...
            print("length of error", len(self.diis_errors))
            self.oldt1 = t1.clone()
            self.oldt2 = t2.clone()
        else:
            # Add DIIS vectors
...
class helper_ldiis(object):
    def __init__(self, no,t1_ii, t2_ij, max_diis):
        ...
        self.oldt1 = oldt1.copy()
        self.oldt2 = oldt2.copy()
    ...
    def add_error_vector(self, no,t1_ii, t2_ij):
        ...
        for i in range(no):
            ...
            self.addt1.append(t1_ii[i].copy())

            for j in range(no):
                ij = i*no + j

                self.addt2.append(t2_ij[ij].copy())
                #if ij == 2:
                    #print("new t2 part 1", ij, t2_ij[ij])

        self.diis_vals_t1.append(self.addt1.copy())
        self.diis_vals_t2.append(self.addt2.copy())
        #print("new t2", self.diis_vals_t2[-1][2])
        #for n1, e1 in enumerate(self.diis_vals_t2):
            #print("n1", n1, "e1", e1[24])

        #print("oldt2", self.oldt2[2])
        ...
        for i in range(no):
             ...
             error_t1.append((self.diis_vals_t1[-1][i] - self.diis_vals_t1[len(self.diis_vals_t1) -2][i]).ravel())
             for j in range(no):
                ij = i*no + j

                #error_t2.append((self.diis_vals_t2[-1][ij] - self.oldt2[ij]).ravel())
                error_t2.append((self.diis_vals_t2[-1][ij] - self.diis_vals_t2[len(self.diis_vals_t2) -2][ij]).ravel())
                #if ij ==2:
                    #print("new t2", self.diis_vals_t2[-1][ij])
                    #print("oldt2",self.oldt2[ij])
                    #print("this should be the old t2", self.diis_vals_t2[len(self.diis_vals_t2) -2][ij])
                    #print("diff", error_t2[ij])
        ...
        #print("length of error", len(self.diis_errors_t1))
        self.diis_errors_t2.append(error_t2)
        self.oldt1 = self.diis_vals_t1[-1].copy()
        self.oldt2 = self.diis_vals_t2[-1].copy()
        #print("this should be old t2", self.oldt2[2])
