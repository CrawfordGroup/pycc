"""
hbar.py: Builds the similarity-transformed Hamiltonian (one- and two-body terms only) in the required format for td-eom-ccsd.
"""
...
class cchbar(object):
    ...
    def __init__(self, ccwfn):
        ...
        self.Hoooo = self.build_Hoooo_static(o, v, ERI, t1, t2)
        self.Hvvvv = self.build_Hvvvv_static(o, v, ERI, t1, t2)
        self.Hvovv = self.build_Hvovv_static(o, v, ERI, t1)
        self.Hooov = self.build_Hooov_static(o, v, ERI, t1)
        self.Hovvo = self.build_Hovvo_static(o, v, ERI, L, t1, t2)
        self.Hovov = self.build_Hovov_static(o, v, ERI, t1, t2)
        self.Hvvvo = self.build_Hvvvo_static(o, v, ERI, L, self.Hov, self.Hvvvv, t1, t2)
        self.Hovoo = self.build_Hovoo_static(o, v, ERI, L, self.Hov, self.Hoooo, t1, t2)
    ...
    """
    def build_Hov(self, o, v, F, L, t1):   # we need to propagate this in time, one-e piece
    ...
    def build_Hvv(self, o, v, F, L, t1, t2):  # we need to propagate this in time, one-e piece
        contract = self.contract
        if self.ccwfn.model == 'CCD':
            if isinstance(F, torch.Tensor):
                Hvv = F[v,v].clone()
    ...
    def build_Hoo(self, o, v, F, L, t1, t2):  # we need to propagate this in time, one-e piece
        contract = self.contract
        if self.ccwfn.model == 'CCD':
            if isinstance(F, torch.Tensor):
                Hoo = F[o,o].clone()
    ...
    def build_Hoooo_static(self, o, v, ERI, t1, t2):  # static only
        contract = self.contract
        if self.ccwfn.model == 'CCD':
    ...
    def build_Hvvvv_static(self, o, v, ERI, t1, t2):
        contract = self.contract
        if self.ccwfn.model == 'CCD':
            if isinstance(ERI, torch.Tensor):
    ...
    def build_Hvovv_static(self, o, v, ERI, t1):
        contract = self.contract
        if self.ccwfn.model == 'CCD':
            if isinstance(ERI, torch.Tensor):
    ...
    def build_Hooov_static(self, o, v, ERI, t1):
        contract = self.contract
        if self.ccwfn.model == 'CCD':
            if isinstance(ERI, torch.Tensor):
    ...
    def build_Hovvo_static(self, o, v, ERI, L, t1, t2):
        contract = self.contract
        if self.ccwfn.model == 'CCD':
            if isinstance(ERI, torch.Tensor):
    ...
    def build_Hovov_static(self, o, v, ERI, t1, t2):
        contract = self.contract
        if self.ccwfn.model == 'CCD':
            if isinstance(ERI, torch.Tensor):
    ...
    def build_Hvvvo_static(self, o, v, ERI, L, Hov, Hvvvv, t1, t2):
        contract = self.contract
        if self.ccwfn.model == 'CCD':
            if isinstance(ERI, torch.Tensor):
    ...
    def build_Hovoo_static(self, o, v, ERI, L, Hov, Hoooo, t1, t2):
        contract = self.contract
        if self.ccwfn.model == 'CCD':
            if isinstance(ERI, torch.Tensor):
