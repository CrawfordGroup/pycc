...
class helper_ldiis(object):
    def __init__(self, no,t1_ii, t2_ij, max_diis, Local):
        ...
        #PNO space to project to 
        self.tmp = 13

        #Project all pno space to one particular space (tmp)  
    ...
    def add_error_vector(self, no,t1_ii, t2_ij):
        ...
        #Porject all pno space to one particular space (tmp) 
        t1 = np.zeros((no,self.dim[self.tmp])) 
        t2 = np.zeros((no,no,self.dim[self.tmp], self.dim[self.tmp]))
    ...
    def extrapolate(self, no, t1_ii, t2_ij):
        ...
        #reverting back to their respective pno space
        for i in range(no):
            ii = i*no + i
