...
class helper_ldiis(object):
    def __init__(self, no,t1_ii, t2_ij, max_diis, Local):
        oldt1 = []
        oldt2 = []
        ...
        t1 = np.zeros((no,self.dim[self.tmp])) 
        t2 = np.zeros((no,no,self.dim[self.tmp], self.dim[self.tmp]))

        for i in range(no): 
            ii = i*no + i

            S = self.QL[self.tmp].T @ self.QL[ii] 
            t1[i] = t1_ii[i].copy() @ S.T 

            for j in range(no):
                ij = i*no + j

                S = self.QL[self.tmp].T @ self.QL[ij]
                t2[i,j] = S @ t2_ij[ij].copy() @ S.T 
                
        self.oldt1 = t1.copy()
        self.oldt2 = t2.copy()
        #print("t2 init", self.oldt2[0,0])
        self.diis_vals_t1 = [t1.copy()]
        self.diis_vals_t2 = [t2.copy()]
        #print("t2 init", self.diis_vals_t2[-1][0,0])
        self.diis_errors = []
        self.diis_size = 0
        self.max_diis = max_diis
    ...
    def add_error_vector(self, no,t1_ii, t2_ij):
        ...
        error_t2 = (self.diis_vals_t2[-1] - self.oldt2).ravel()
        self.diis_errors.append(np.concatenate((error_t1, error_t2)))
        #print("previous t2_add", self.oldt2[0,0])
        self.oldt1 = t1.copy()
        self.oldt2 = t2.copy()
        #print("incoming t2_add",self.oldt2[0,0])
    ...
    def extrapolate(self, no, t1_ii, t2_ij):
        ...
        # Save extrapolated amplitudes to old_t amplitudes
        #print("incoming t2_extra", self.oldt2[0,0])
        self.oldt1 = t1.copy()
        self.oldt2 = t2.copy()
        #print("updated t2_extra", t2[0,0])
        #print("should updated t2_extra", self.oldt2[0,0])
        t1_ii = []
        t2_ij = []
