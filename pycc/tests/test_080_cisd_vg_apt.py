"""
CISD velocity-gauge atomic polar tensors - pycc.apt(pycc.CIderiv(ci), gauge='velocity') facade. velocity_dipole_derivatives() is CISD-specific (a wave-function-overlap construction the base does not provide), -2*(Icc+Icphi+Iphic+Iphiphi) with the vector-potential CPCI response.
...
def test_cisd_vg_apt_vs_reference(cisd_h2o2):
    ...
    element."""
    P = np.asarray(pycc.apt(pycc.CIderiv(cisd_h2o2()), gauge='velocity').total).reshape(-1, 3)
